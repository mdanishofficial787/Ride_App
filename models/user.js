...
UserSchema.methods.generateEmailVerficationOTP = function () {
    const otp = Math.floor(100000 + Math.random() * 900000).toString();  // 6 digit otp generation

    // hashing otp and storing hashed value in emailVerficationOTP field
    this.emailVerficationOTP = crypto
        .createHash('sha256')
        .update(otp)
        .digest('hex');
